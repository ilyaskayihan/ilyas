...
classCheck <- function(functionResult, class, called.with) {
  ...
  if(test){
    ...
    but should be: %s\n
    %s", class(functionResult), class, called.with))
  }
}
...
expect_function_equal <- function(fun, inputs, .expected, fname = NULL) {
  if(is.null(fname)) fname <- as.character(substitute(fun))

  called.with <- make.Called.with(fun, inputs, fname)

  withCallingHandlers(functionResult <- do.call(fun, inputs),
                      error = function(e) {
